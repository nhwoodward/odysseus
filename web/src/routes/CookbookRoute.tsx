  ...
  return <section data-tour="cookbook-discovery">
    ...
    <div className="overflow-hidden rounded-lg border bg-card">
      ...
      {source === "fit" ? <>
        ...
        <div className="max-h-[520px] overflow-auto"><table className="w-full min-w-[760px] caption-bottom text-sm"><TableHeader className="sticky top-0 z-10 bg-card [&_th]:h-9"><TableRow><TableHead>Model</TableHead><TableHead>Fit</TableHead><TableHead>Quant</TableHead><TableHead>Memory</TableHead><TableHead>Speed</TableHead><TableHead>Context</TableHead><TableHead className="pr-3 text-right"></TableHead></TableRow></TableHeader><TableBody>{(fit.data?.models || []).map((model: HwfitModel) => <TableRow key={model.name}><TableCell><div className="font-medium">{model.name}</div><div className="text-xs text-muted-foreground">{model.parameter_count || (model.params_b ? `${model.params_b}B` : "")} {model.use_case ? `· ${model.use_case}` : ""}</div></TableCell><TableCell><Badge className={cn("border-transparent font-normal capitalize", fitTone(model.fit_level))}>{(model.fit_level || "unknown").replace('_', ' ')}</Badge></TableCell><TableCell>{model.quant || "—"}</TableCell><TableCell>{model.required_gb != null ? `${model.required_gb} GB` : "—"}</TableCell><TableCell>{model.speed_tps != null ? `${Math.round(model.speed_tps)} tok/s` : "—"}</TableCell><TableCell>{model.context ? model.context.toLocaleString() : "—"}</TableCell><TableCell className="pr-3 text-right"><Button size="sm" variant="outline" disabled={download.isPending} onClick={() => downloadRepo(model.name, "hf")}>Download</Button></TableCell></TableRow>)}</TableBody></table>{fit.isLoading && <p className="p-4 text-sm text-muted-foreground">Ranking models for this hardware…</p>}{fit.data?.error && <p className="p-4 text-sm text-destructive">{fit.data.error}</p>}</div>
